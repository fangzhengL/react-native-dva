    ...
    return (
      ...
      >
        <TouchableWithoutFeedback
          ...
          onPress        = { () => this.props.onPress(this.props.inactive) }>
...
class SortableGrid extends Component {
  ...
  assessGridSize = ({nativeEvent}) => {
    ...
    if (this.state.gridLayout != nativeEvent.layout) {
      this.setState({
        ...
        this.reAssessGridRows(true);
      })
    }
  }

  reAssessGridRows = (bIsAssessGridSize) => {
    let oldRows = this.rows
    this.rows = Math.ceil(this.items.length / this.itemsPerRow)
    if (bIsAssessGridSize && this.gridHeightTarget === null) {
  ...
  handleTap = ({ onTap = NULL_FN, onDoubleTap = NULL_FN }) => (inactive) => {
    if (inactive) {
      onTap()
      return
    }
  ...
  deleteBlocks = (deleteBlockIndices) => {
    ...
    this.setState({ blockPositions, blockPositionsSetCount }, () => {
      this.items.forEach( (item, order) => {
        ...
        let thisPosition = { x, y }
        if (!this.state.blockPositions[blockIndex]) {
          this.state.blockPositions[blockIndex] = {
            currentPosition : new Animated.ValueXY( thisPosition ),
            origin          : thisPosition
          }
        } else {
  ...
  _animateGridHeight = () => {
    // Alert.alert('_animateGridHeight', this.blockHeight + '')
  ...
  _getDynamicOpacity = (key) =>
    this._getBlock( key )&&(   this._getBlock( key ).currentPosition.y._value
  ...
  _getBlockStyle = (key) => [
    ...
    this._getBlock(key)&&this._blockPositionsSet() && (this.initialDragDone || this.state.deleteModeOn) &&
    { position: 'absolute',
