...
import {logo} from "@/pages/find/course-detail/assets";
import {tabButton} from "@/pages/customer/component";
import {isIos} from "@/utils";
import {backIcon, backWhiteIcon} from "@/assets/images/customer";
import TouchableItem from "@/components/touchable_item";
...
export default class VideoPlayer extends Component {
  ...
  onProgress(event) {
    ...
    const progress = event.currentTime / (this.props.duration || this.state.duration)
    this.setState({
      progress,
    });
    return {progress, duration: (this.props.duration || this.state.duration)}
  }
  ...
  onLoad(event) {
    ...
    const {seekT, progress} = this.props;
    if (seekT) {
      this.setState(
        {
          duration,
    ...
    return (
      <View style={[styles.controls, customStyles.controls]}>
        ...
        )}
        {
          ...
          (
            <TouchableOpacity
              onPress={isIos ? this.onToggleFullScreen : this.props.onToggleFullScreen}
  ...
  renderVideo() {
    ...
    return (
      <View style={customStyles.videoWrapper}>
          ...
          />
          {
            ...
            (
              <View style={{
                ...
                <Image source={logo}/>
                <Text style={{color: 'white'}}>正在缓冲中,请稍等</Text>
              </View>
            )
          }
        </View>
        ...
        {
          ((!this.state.isPlaying) || this.state.isControlsVisible) && this.props.goBack ?
            <TouchableItem
              style={{
                width: 50,
                height: 50,
              ...
              <Image style={{alignSelf: 'center', width: 20, height: 20}} source={backWhiteIcon} resizeMode='center'/>
            </TouchableItem> : null
        }
      </View>
    );
